...
test('breadboard: power rails run the full length of the board', () => {
  const net = new Netlist();
  const spec: DiagramPart = { type: 'circuitlab-breadboard', id: 'bb', attrs: {} };
  breadboard(makeCtx(net), spec);

  net.drive('bb:tp1', { v: 5, strong: true });
  assert.equal(net.value('bb:tp30'), 5); // + rail tied end to end
  assert.equal(Number.isNaN(net.value('bb:tn1')), true); // - rail is a separate node

  net.drive('bb:bn1', { v: 0, strong: true });
  assert.equal(net.value('bb:bn30'), 0);
  assert.equal(Number.isNaN(net.value('bb:bp1')), true); // rails don't cross-connect to each other
});
